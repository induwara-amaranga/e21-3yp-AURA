...
class RestaurantRobot:
    ...
    def __init__(self, max_run_seconds=None, auto_stop_on_lock=False, minimize_window=False, home_on_exit=False):
        self._stop_event = threading.Event()
        ...
        if SHOW_WINDOW:
            ...
            cv2.resizeWindow(WINDOW_NAME, WIN_W, WIN_H)
            cv2.moveWindow(WINDOW_NAME, 0, 0)
            # Optionally start minimized/hidden so it does not steal focus.
            try:
                if self.minimize_window:
                    # Try hiding the window using OpenCV window property.
                    cv2.setWindowProperty(WINDOW_NAME, cv2.WND_PROP_VISIBLE, 0)
            except Exception:
                # Some OpenCV builds/backends may not support this; ignore.
                pass
                    ...
                    with self._lock:
                        if centred:
                            if self.stable_since is None:
                                ...
                                self.frozen = True
                                # If requested, auto-stop when lock achieved.
                                if self.auto_stop_on_lock:
                                    log.info("Auto-stop on lock requested: stopping tracker.")
                                    # signal threads to stop; main loop will cleanup
                                    self._stop_event.set()
                        else:
                            self.stable_since = None
            else:
    ...
    def run(self):
        ...
        try:
            while not self._stop_event.is_set():
                ...
                # If max run time set, stop when elapsed
                if self.max_run_seconds is not None and (time.time() - self.start_time) >= self.max_run_seconds:
    ...
    def cleanup(self):
        ...
        # Optionally move servo to home AFTER threads stop.
...
def _parse_args():
    ...
    p.add_argument("--max-run-seconds", type=float, default=os.getenv("FACE_TRACKER_MAX_SECONDS", None), help="auto-stop after N seconds (env FACE_TRACKER_MAX_SECONDS)")
    p.add_argument("--auto-stop-on-lock", action="store_true", help="stop automatically when face lock is achieved")
    p.add_argument("--minimize-window", action="store_true", help="start display window minimized/hidden")
    p.add_argument("--home-on-exit", action="store_true", help="return servos to home on exit (default: stay at last position)")
    return p.parse_args()


if __name__ == "__main__":
    args = _parse_args()
    # If env var provided as string '30', argparse default will be string; ensure numeric or None
    max_seconds = None
    ...
    robot = RestaurantRobot(max_run_seconds=max_seconds, auto_stop_on_lock=args.auto_stop_on_lock, minimize_window=args.minimize_window, home_on_exit=args.home_on_exit)
    robot.run()
